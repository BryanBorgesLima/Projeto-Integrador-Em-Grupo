...
if (typeof MESAS === "undefined") { var MESAS = []; }
...
function openMesaModal(onSelect) {
  ...
  const options = (MESAS.length ? MESAS.map((m) => m.numero) : Array.from({ length: 12 }, (_, i) => i + 1));
  overlay.innerHTML = `
...
async function callWaiter(btn) {
  ...
  try {
    const response = await fetch("../api/chamar_garcom.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ mesa }),
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok || !data.sucesso) throw new Error(data.erro || "Não foi possível chamar o garçom.");
    showToast("Garçom chamado! Já estamos a caminho da sua mesa.");
  } catch (err) {
    showToast(err.message || "Não foi possível chamar o garçom agora.");
  } finally {
    setTimeout(() => {
      btn.disabled = false;
...
/* ===================== Status do pedido (indicador no cabeçalho + página dedicada) ===================== */
const STATUS_LABELS_CLIENT = { preparando: "Preparando", pronto: "Pronto", entregue: "Entregue" };
...
async function fetchPedidoAtual() {
  ...
  if (!mesa) return null;
  try {
    const response = await fetch(`../api/pedido_mesa.php?mesa=${mesa}`);
...
function trackerEmptyHTML(message) {
  return `
    <div class="tracker-empty">
      ...
      <p>${message}</p>
      <a href="menu.php" class="btn btn-primary" style="margin-top:14px;">Ver cardápio</a>
    </div>`;
}

function initStatusPage() {
  renderStatusPage();
  setInterval(renderStatusPage, 8000);
}
  ...
  const itemsHTML = pedido.itens
    .map((it) => {
      ...
      <div class="tracker-item-row">
        <span class="tracker-item-info">
          <span class="tracker-item-thumb">${imageTagHTML(it.imagem, it.nome)}</span>
          <span>
...
function initMenuPage() {
  ...
  // Link direto vindo de produto.php (?open=ID) reabre o modal daquele item.
...
function renderProductGrid() {
  ...
  // Destaques da casa aparecem primeiro dentro de cada categoria.
  ...
  items.forEach((product) => {
    ...
    card.innerHTML = `
      <div class="thumb">
        ${product.featured ? `<span class="featured-badge">Destaque da casa</span>` : ""}
    ...
    <div class="product-modal">
      ...
      ? `<div class="modal-section">
                ...
                <div class="pill-row" id="ingredientPills">
                  ${product.ingredients
        .map((ing) => `<button type="button" class="ingredient-pill" data-ing="${ing}"><span class="label">${ing}</span></button>`)
        .join("")}
                </div>
              </div>`
      : ""
      ...
      ? `<div class="modal-section">
                ...
                <div class="pill-row" id="extraPills">
                  ${productExtras.map(
        (extra) => `<button type="button" class="extra-pill" data-extra="${extra.id}">${extra.name} (+${formatPrice(extra.price)})</button>`
      ).join("")}
                </div>
              </div>`
      : ""
