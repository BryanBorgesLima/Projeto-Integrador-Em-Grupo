// Persistência do carrinho e da mesa atual via localStorage.
...
// customization = { removedIngredients: string[], extras: {chave: qty}, notes: string }
...
function addToCart(productId, qty = 1, customization = {}) {
  ...
  const existing = cart.find(
    (item) => item.productId === productId && customizationKey(item) === key
  );
  if (existing) {
    existing.qty += qty;
  } else {
...
// Descrição legível das customizações de um item de carrinho, usada em vários lugares.
